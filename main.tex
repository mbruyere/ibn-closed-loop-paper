...
\acmConference[IBN-CL '26]{Workshop on Intent-Based Networking and
  Closed-Loop Control}{2026}{TBD}
\acmYear{2026}
\copyrightyear{2026}
\acmDOI{}
\acmISBN{}
...
\begin{enumerate}[label=\textbf{AC\arabic*.}]
  \item \textbf{A single external trigger.} The only event that
    ...
    point \texttt{HldCommitPipeline.run(commit, hld\_path)}, which is
  ...
  \item \textbf{A single human checkpoint.} The inner loop is
    ...
    deposited under \texttt{.git/ibn-pending-approval/<sha>.md}
...
(\texttt{Enterprise\_Campus\_Network\_HLD.md}), the \texttt{post-commit}
hook spawns \texttt{HldCommitPipeline.run(commit\_sha, hld\_path)}.
...
_KIND_MAP = {
  ...
  \item \textbf{Approval gate.}
    ...
    \texttt{.git/ibn-pending-approval/<sha>.md} and halts. The
...
python -m ibn.tools.approve <sha>
    ...
    \texttt{HldCommitPipeline.resume\_from\_approval(plan\_id)}.
...
def _resolve_executor(platform):
  ...
  \item \textbf{Agent~7 (Assessment).}
    ...
    \texttt{REALIZED\_BY}/\texttt{AGGREGATES} edges) to identify
...
1 & HLD Pipeline &
  ...
  52\\
2 & Multi-vendor &
  SR~Linux template chain (\texttt{srl\_base}, \texttt{srl\_interfaces},
  \texttt{srl\_vlans}), \texttt{docker exec sr\_cli} executor,
  pioneer switch \texttt{acc-sw-01} push &
  72\\
3 & Provisioning &
  Containerlab YAML mutation, \texttt{provision()} and
  \texttt{retire()}, \texttt{\_KIND\_MAP} dispatch, lifecycle events &
  86\\
4 & Approval gate &
  A4 severity classifier, \texttt{MigrationPlan} node, pending-plan
  Markdown, approve/reject CLI tool, \texttt{resume\_from\_approval()},
  auto-approve for LOW/INFO &
  100\\
5 & FRR + batch &
  FRR as third vendor (Dockerfile, 3 templates, vtysh executor),
  \texttt{provision\_batch()} fix, 4 new devices provisioned,
  VM resize to 12\,GB &
  118\\
7a & gNMI transport &
  \texttt{pygnmi} executor for SR~Linux, CLI$\rightarrow$YANG
  translator (10 rules), \texttt{IBN\_SRLINUX\_TRANSPORT} env flag,
  live \texttt{SetRequest} + verification &
  140\\
\bottomrule
\end{tabular}
\end{table}
